...
def bright_ideas():
    if session.get('id') == None:
        ...
        likes = Post.get_amount_of_likes_by_post()        
        print("\n\n\n",posts)
        return render_template("user_page.html",user=usuario,ideas=ideas,likes=likes)
...
def user_page(id):
    ...
    cantLikes = Post.get_amount_of_likes(id)
    cantLikes = cantLikes[0]['count(user_id)']
    print("\n\n",cantLikes)

    return render_template("user_summary.html",users=usuario,ideas=ideas,id=id,likes=likes,usu=user,total=total,cantLikes = cantLikes)